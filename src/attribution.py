...
def main() -> None:
    ...
    if verdict_path.exists():
        sentences["verdict"] = pd.read_csv(verdict_path)["verdict"].to_numpy()
    else:
    ...
    lines = [
        ...
        "",
        "The narrow row is the one comparable to the **53.5%** recorded in "
        "`pre-delib_q_statements_depersonalised.CHANGES.md`, which came from a "
        "narrower ad-hoc pattern run before this script existed. The two are "
        "measuring the same thing at different sensitivities, so they should be "
        "in the same region rather than identical.",
        "",
